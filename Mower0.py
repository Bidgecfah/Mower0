...
#         日志.info(f'Start debug mode, log is stored in {config.LOGFILE_PATH}')
#         init_fhlr(运行信息滚动窗)
...
class 项目经理(BaseSolver):
    ...
    def transition(self) -> None:
        self.recog.update()
        if self.get_infra_scene() == 1: self.tap_index_element('infrastructure')
    ...
    def 处理报错(self, force=False):
        ...
        while self.scene() == -1:
            ...
            if (datetime.now() - 报错计时).total_seconds() > self.跑单提前运行时间 // 4:
                日志.error(f'报错次数达{循环次数}次，时间长达{round((datetime.now() - 报错计时).total_seconds())}秒')
                while not Mower0线程.stopped():
                    try:
                        Mower0线程._stop_event.set()
                        终止线程报错(Mower0线程.ident, SystemExit)
                    except: pass
                日志.warning('Mower0已停止，准备重新启动Mower0')
                Mower0线程 = 线程()
                Mower0线程.start()
                return
            self.recog.update()
        if self.error or force:
            ...
            if self.find_next_task(datetime.now()) is None:
                日志.debug("由于出现错误情况，生成一次空任务来执行纠错")
                    ...
                    for i in range(0, length):
                        ...
                        if not 读取到的干员名 == '':
                            if 读取到的干员名 not in self.干员信息.operators.keys() and 读取到的干员名 in agent_list:
                                ...
                                该贸易站干员名单 += 读取到的干员名
                    日志.info(f'记录干员工位：{该贸易站干员名单}')
        # 准备数据
        日志.debug(self.干员信息.print())
    ...
    def 读取接单时间(self, 门牌号):
        if Mower0线程.stopped(): return
        日志.info(f'读取贸易站 B{门牌号[5]}0{门牌号[7]} 接单时间')
        # 进入房间详情
        报错计数 = 0
        ...
        日志.info(f'贸易站 B{门牌号[5]}0{门牌号[7]} 接单时间为 {接单等待时间.strftime("%H:%M:%S")}')
    ...
    def 读取时间(self, 位置范围, 上限, 报错计数=0, 读数器=False):
        ...
        try:
            ...
            if 报错计数 > 10:
                日志.exception(f"读取失败{报错计数}次超过上限")
                return None
    ...
    def 进入房间(self, 门牌号: str) -> tp.Rectangle:
        ...
        while not 已进入房间:
            try:
                ...
                if 门牌号.startswith('room'): 日志.info(f'进入房间 B{门牌号[5]}0{门牌号[7]}')
                elif 门牌号 == 'dormitory_4': 日志.info('进入房间 B401')
                else: 日志.info(f'进入房间 B{门牌号[10]}04')
                已进入房间 = True
            except Exception as e:
                尝试进入次数 += 1
                self.返回基建主界面()
    ...
    def get_arrange_order(self) -> 干员排序方式:
        ...
        日志.debug((best_score, best_order))
        return best_order
    ...
    def 筛选器(self, 开, 模式="未进驻"):
        日志.info(f'开始 {("打开" if 开 else "关闭")} {模式} 筛选')
    ...
    def 换上干员(self, 换上干员列表: list[str], 门牌号: str) -> None:
        ...
        if 门牌号.startswith('room') and ('但书' in 当前换上干员列表 or '龙舌兰' in 当前换上干员列表):
            日志.info(f'{换上干员名单} 进驻房间 B{门牌号[5]}0{门牌号[7]} 时间为 {(self.任务列表[0].time + timedelta(seconds=(self.跑单提前运行时间 - self.更换干员前缓冲时间))).strftime("%H:%M:%S")}')
        else: 日志.info(f'换上 {换上干员名单}')
        刚进入干员选择界面 = True
        右移次数 = 0
        重试计数 = 0
        # 如果重复进入宿舍则需要排序
    ...
    def 撤下干员(self, 门牌号, 读取时间参数=None):
        ...
        for _operator in self.干员信息.operators.keys():
            if self.干员信息.operators[_operator].current_room == 门牌号 and _operator not in [res['agent'] for res in 结果]:
                ...
                撤下干员名单 += _operator
        if not 撤下干员名单 == '撤下': 日志.info(撤下干员名单)
        return 结果
    ...
    def 检查换人情况(self, 房间, 任务列表, 获取时间=False):
        ...
        for 序号, 干员名 in enumerate(任务列表[房间]):
            if not 当前房间干员列表[序号]['agent'] == 干员名:
                日志.error(f'检测到的干员{当前房间干员列表[序号]["agent"]},需要上岗的干员{干员名}')
        ...
        for 房间 in 任务房间列表:
            ...
            while not 干员已进驻:
                try:
                    ...
                    if 房间.startswith('room') and 用户配置['跑单消耗无人机开关'] == '关':
                        ...
                        if 龙舌兰_但书进驻前的等待时间 > 0:
                            日志.info(f'龙舌兰、但书进驻前等待 {str(龙舌兰_但书进驻前的等待时间)} 秒')
            ...
            if 房间.startswith('room'):
                ...
                if (等待时间 > 0) and (等待时间 < self.跑单提前运行时间 * 2):
                    try:
                        if 用户配置['跑单消耗无人机开关'] == '关':
                            日志.info(f'房间 B{房间[5]}0{房间[7]} 修正后的接单时间为 {修正后的接单时间.strftime("%H:%M:%S")}')
                            日志.info(f'等待截图时间为 {str(等待时间)} 秒')
                            time.sleep(等待时间)
                        else: self.无人机协助跑单()
                        self.recog.update()
                        while self.find("order_ready", scope=((self.recog.w * 450 // 1920, self.recog.h * 675 // 1080), (self.recog.w * 600 // 1920, self.recog.h * 750 // 1080))) is None:
                            ...
                            self.recog.update()
                        日志.info('跑单成功')
                    except Exception as e: 日志.exception(e)
                else: 日志.debug('检测到漏单！')
                日志.info('保存截图')
                if self.get_infra_scene() == 9:
                    if not self.waiting_solver(9, sleep_time=2, 服务器=服务器): return
    ...
    def MAA日志(MAA日志, details, arg):
        MAA日志信息 = Message(MAA日志)
        MAA日志内容 = json.loads(details.decode('utf-8'))
        日志.debug(MAA日志内容)
        日志.debug(MAA日志信息)
        日志.debug(arg)
        if "what" in MAA日志内容 and MAA日志内容["what"] == "StageDrops":
            global 关卡掉落
            关卡掉落["details"].append(MAA日志内容["details"]["drops"])
            关卡掉落["summary"] = MAA日志内容["details"]["stats"]
            for 种类 in MAA日志内容["details"]["drops"]:
                物品名称 = 种类["itemName"]
                物品数量 = 种类["quantity"]
                日志.info(f"关卡掉落: {物品名称} × {物品数量}")
    ...
    def MAA初始化(self):
        ...
        self.关卡列表 = []
        if self.MAA.connect(self.MAA设置['MAA_adb路径'], self.device.client.device_id): 日志.info("MAA 连接成功")
        else:
            日志.info("MAA 连接失败")
    ...
    def MAA任务调度器(self, 任务列表=['Fight'], 首次=False):
        try:
            ...
            if self.MAA设置['集成战略'] == '开' or self.MAA设置['生息演算'] == '开':
                ...
                self.发送邮件('MAA 停止')
                if 强制停止MAA:
                    日志.info(f"MAA 任务未完成，等待3分钟重启软件")
                    time.sleep(180)
                    self.device.exit(self.服务器)
                elif not 首次:
                    日志.info(f"记录 MAA 本次执行时间:{datetime.now()}")
                    # 有掉落东西再发
            ...
            if 任务倒计时 > 0:
                日志.info(f"休息{int(任务倒计时)}秒，到{下个任务开始时间}")
                time.sleep(任务倒计时)
            self.MAA = None
        except Exception as e:
            日志.error(e)
            ...
            任务倒计时 = (下个任务开始时间 - datetime.now()).total_seconds()
            if 任务倒计时 > 0:
                日志.info(f"休息{int(任务倒计时 / 60)}分钟，到{下个任务开始时间}")
                time.sleep(任务倒计时)
...
def 初始化(任务列表, scheduler=None):
    ...
    device = Device()
    cli = Solver(device)
    PRTS干员上线时间一览 = "https://prts.wiki/w/干员上线时间一览"
    # 找到含有指定字符串的表格
    干员表格 = None
    干员列表 = []
    try:
        全部表格 = BeautifulSoup(requests.get(PRTS干员上线时间一览).text, "html.parser").find_all("table")
        for 表格 in 全部表格:
            if "主要获得方式" in 表格.text:
                干员表格 = 表格
                break
        if 干员表格:
            # # 提取表头
            # 表头 = [header.text.strip() for header in 干员表格.find_all("a")]
            # 提取表格数据
            for 行 in 干员表格.find_all("tr"):
                干员列表.append(行.contents[1].text)
            干员列表文件地址 = Path(__rootdir__) / 'data' / 'agent.json'
            with open(干员列表文件地址, 'r', encoding='utf-8') as 原干员列表文件:
                原干员列表 = json.load(原干员列表文件)
            if 干员列表 != 原干员列表:
                with open(干员列表文件地址, "w", encoding="utf-8") as 干员列表文件:
                    json.dump(干员列表, 干员列表文件, ensure_ascii=False, indent=4)
                    日志.info('干员列表已更新')
    except Exception: pass
    if scheduler is None:
        当前项目 = 项目经理(cli.device, cli.recog)
        日志.info(f'当前模拟器分辨率为 {当前项目.recog.w} × {当前项目.recog.h}')
        if not 当前项目.recog.w * 9 == 当前项目.recog.h * 16:
            日志.error('请将模拟器分辨率设置为 16:9 再重新运行 Mower0!')
            ...
            try:
                if len(当前项目.任务列表) > 0:
                    ...
                    if (len(任务列表) > 1 and (任务列表[0].time - datetime.now()).total_seconds()
                        ...
                        日志.warning("两个订单时间太接近了，准备用无人机协助拉开订单间时间差距")
                    ...
                    if (len(任务列表) > 1 and 2 * 当前项目.跑单提前运行时间 >
                        ...
                        日志.warning("跑单时间与4:00太接近了，准备用无人机协助提前订单时间")
                        当前项目.无人机加速调整订单时间(任务列表[0].type, True, True)
                    if (len(任务列表) > 1 and 2 * 当前项目.跑单提前运行时间 >
                            (datetime.now().replace(hour=16, minute=0, second=0, microsecond=0)
                             - 当前项目.任务列表[0].time).total_seconds() > 0):
                        日志.warning("跑单时间与16:00太接近了，准备用无人机协助提前订单时间")
                        当前项目.无人机加速调整订单时间(任务列表[0].type, True, True)
                    ...
                    if 任务间隔 > 0:
                        ...
                        if MAA设置['作战开关'] == '开' and (任务间隔 > 600): 当前项目.MAA任务调度器()
                            ...
                            日志.info(f'房间 B{任务列表[0].type[5]}0{任务列表[0].type[7]} 开始跑单')
...
def 开始运行():
    ...
    with open('Mower0用户配置文件.yaml', 'r', encoding='utf-8') as 用户配置文件:
        运行配置 = yaml.load(用户配置文件.read(), Loader=yaml.FullLoader)
    服务器 = 'com.hypergryph.arknights.bilibili' if 运行配置['服务器'] == 'Bilibili服务器' else 'com.hypergryph.arknights'
    弹窗提醒 = True if 运行配置['弹窗提醒开关'] == '开' else False
    跑单提前运行时间 = 运行配置['跑单提前运行时间']
    更换干员前缓冲时间 = 运行配置['更换干员前缓冲时间']
    自动休息 = True if 运行配置['自动休息'] == '开' else False
    悬浮字幕开关 = True if 运行配置['悬浮字幕开关'] == '开' else False
    签到 = True if 运行配置['森空岛签到开关'] == '开' else False
    森空岛小秘书 = True if 运行配置['森空岛小秘书开关'] == '开' else False
    if not 运行配置['字幕字号'] == '默认': 字幕字号 = int(运行配置['字幕字号'])
    字幕颜色 = 运行配置['字幕颜色']
    邮件设置 = 运行配置['邮件设置']
    MAA设置 = 运行配置['MAA设置']
    config.ADB_DEVICE = [运行配置['adb地址']]
    config.ADB_CONNECT = [运行配置['adb地址']]
    config.APPNAME = 服务器
    if 初启动:
        Mower0线程 = 线程()
        Mower0线程.start()
...
def 延迟运行Mower0(delay_seconds: float = 0, *args, **kwargs):
    global 延迟运行Mower0线程
    停止运行Mower0()
    日志.warning(f'{delay_seconds // 60} 分钟后重新运行Mower0')
...
def 停止运行Mower0():
    ...
    try:
        ...
        pass
    日志.warning('Mower0已停止')
    基础运行设置.grid(row=0, column=0, sticky=界面.W + E + N + S, padx=10, pady=10)
    跑单位置设置.grid(row=1, column=0, sticky=界面.W + E + N + S, padx=10, pady=10)
        ...
        for i in characters:
            ...
            if resp['code'] == 0:
                ...
                for j in resp['data']['awards']:
                    res = j['resource']
                    日志.warning(f'获得了{res["name"]} × {j.get("count") or 1}')
                    if 弹窗提醒: 托盘图标.notify(
                        f'{i.get("nickName")}({i.get("channelName")})在森空岛签到成功！\n获得了{res["name"]} × {j.get("count") or 1}',
                        "森空岛签到")
            elif resp['code'] == 10001:
                已签到日期 = datetime.now().strftime('%Y年%m月%d日')
                日志.info(f'今天是{已签到日期}，{i.get("nickName")}({i.get("channelName")})今天在森空岛已经签过到了')
            else:
                日志.warning(f'{i.get("nickName")}({i.get("channelName")})签到失败了！原因：{resp.get("message")}')
                if 弹窗提醒:
                    托盘图标.notify(
                        f'{i.get("nickName")}({i.get("channelName")})签到失败了！原因：{resp.get("message")}', 森空岛签到)
                continue
    except Exception as ex:
        日志.warning(f'森空岛签到失败，原因：{str(ex)}')
        日志.error('', exc_info=ex)
...
def 森空岛干员阵容查询():
    try: 森空岛获取信息()
    except Exception as ex: 日志.warning(f'森空岛信息获取失败，原因：{str(ex)}')
    数据 = json.load(open('森空岛数据.json'))['data']
    ...
    运行信息滚动窗.yview(END)  # 自动滚动到底部
    日志.warning(f"总计消耗龙门币 {总计消耗龙门币} / 经验 {总计消耗经验} = {round(总计消耗龙门币 / 总计消耗经验, 3)}")


def 设置主界面透明度(不透明度): 窗口.attributes("-alpha", 1-0.9*float(不透明度))


def 主界面置于顶层设置(): 窗口.attributes("-topmost", 主界面置于顶层开关输入.get())


def 主界面置于顶层(): 窗口.attributes("-topmost", True)
...
def 运行信息区域自适应(event=None):
    窗口高 = 窗口.winfo_height()
    运行信息滚动窗.config(height=max(窗口高//运行信息滚动窗高分数-12, len(跑单位置表)*运行信息滚动窗高系数))


def 运行信息滚轮(event): 运行信息滚动窗.yview_scroll(int(-1*(event.delta/120)), "units")
...
def 拖动字幕窗口(event): 字幕窗口.geometry(f'+{event.x_root - 鼠标水平初始位置}+{event.y_root - 鼠标竖直初始位置}')


def 关闭字幕窗口(icon: pystray.Icon): 字幕窗口.withdraw()
...
窗口 = 界面.Window(title="Mower0", themename="minty", iconphoto="元素/图标.png", size=(1300, 900), minsize=(0, 0))
窗口.attributes("-alpha", 1)
...
跑单位置设置.grid(row=1, column=0, rowspan=2, sticky=界面.W+E+N+S, padx=10, pady=10)
...
森空岛干员阵容查询按钮 = 界面.Button(滚动区域[0], text="森空岛干员阵容查询", bootstyle=(PRIMARY, "outline-toolbutton"), command=森空岛干员阵容查询, width=按钮宽度)
森空岛干员阵容查询按钮.grid(row=0, column=1, padx=10, pady=23, ipady=30, sticky=界面.N+S)
keyboard.add_hotkey('alt+k', 开始运行)
开始运行按钮 = 界面.Button(滚动区域[0], text="开始运行", bootstyle=(PRIMARY, "outline-toolbutton"), command=开始运行, width=按钮宽度)
开始运行按钮.grid(row=0, column=3, padx=10, pady=23, ipady=30, sticky=界面.N+S)
keyboard.add_hotkey('alt+t', 停止运行Mower0)
停止运行按钮 = 界面.Button(滚动区域[0], text="停止运行", bootstyle=(SECONDARY, "outline-toolbutton"), command=停止运行Mower0, width=按钮宽度)
停止运行按钮.grid(row=0, column=4, padx=10, pady=23, ipady=30, sticky=界面.N+S)

显示设置 = 界面.Frame(滚动区域[0])
# 显示设置.grid(row=1, column=1, columnspan=4, pady=20, sticky=界面.W+E)
显示设置.grid(row=1, column=1, columnspan=4, sticky=界面.W+E)
界面.Label(显示设置, text="主界面透明度").grid(row=0, column=0, padx=10, sticky=界面.W)
主界面透明度输入 = 界面.Scale(显示设置, from_=0, to=1, length=400, command=设置主界面透明度) ###
主界面透明度输入.grid(row=0, column=1, padx=10, sticky=界面.W)
界面.Separator(显示设置, bootstyle="light").grid(row=0, column=2, padx=5)
主界面置于顶层开关输入 = 界面.BooleanVar(value=FALSE) ###
界面.Checkbutton(显示设置, bootstyle="round-toggle", text="主界面置于顶层", variable=主界面置于顶层开关输入, onvalue=TRUE, offvalue=FALSE, command=主界面置于顶层设置).grid(row=0, column=3, padx=10, sticky=界面.E)

运行信息滚动窗 = 界面.ScrolledText(滚动区域[0], width=65, height=70, wrap=界面.WORD, font=('Consolas', 11), relief=界面.RIDGE)
运行信息滚动窗.vbar.pack_forget()
运行信息滚动窗.grid(row=2, column=1, rowspan=3, columnspan=4, sticky=界面.W+E+N+S, padx=10, pady=10)
运行信息滚动窗.bind("<MouseWheel>", 运行信息滚轮)
窗口.bind("<Configure>", 运行信息区域自适应)
...
if 悬浮字幕开关:
    ...
    悬浮字幕.bind("<Button-1>", 选中字幕窗口)
    悬浮字幕.bind("<B1-Motion>", 拖动字幕窗口)
    悬浮字幕.bind("<Double-Button-1>", 关闭字幕窗口)
